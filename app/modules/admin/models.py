...
class Employee(Base):
    ...
    # 관계
    holidays: Mapped[list["Holiday"]] = relationship("Holiday", back_populates="employee", cascade="all, delete-orphan")

# 공휴일(직원별 쉬는 날 등록)
class Holiday(Base):
    __tablename__ = "holidays"
    __table_args__ = (
        UniqueConstraint("employee_id", "holiday_date", name="uq_holiday_employee_date"),
...
class InsuranceRate(Base):
    ...
    effective_date: Mapped[date] = mapped_column(Date, nullable=False)  # 시행일(동일일자 중복 불가)
