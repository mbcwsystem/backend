...
# ---- Users ----
@router.post("/users/create", response_model=schemas.UserOut, status_code=status.HTTP_201_CREATED)
def create_user(payload: schemas.UserCreate, db: Session = Depends(get_db), _admin=Depends(get_current_admin)):
    try:
...
def list_users(q: Optional[str] = Query(None), limit: int = Query(20, ge=1, le=100), offset: int = Query(0, ge=0),
...
def update_user(memberId: int = Path(..., ge=1), payload: schemas.UserUpdate = ...,
                db: Session = Depends(get_db), _admin=Depends(get_current_admin)):
    try:
...
def delete_user(memberId: int = Path(..., ge=1), db: Session = Depends(get_db), _admin=Depends(get_current_admin)):
    try:
...
def create_holiday(payload: schemas.HolidayCreate, db: Session = Depends(get_db), _admin=Depends(get_current_admin)):
    try:
...
def list_holidays(start: Optional[str] = Query(None), end: Optional[str] = Query(None),
                  db: Session = Depends(get_db), _admin=Depends(get_current_admin)):
    return services.list_holidays(db, start, end)

@router.patch("/holidays/{holidayId}", response_model=schemas.HolidayOut)
def update_holiday(holidayId: int = Path(..., ge=1), payload: schemas.HolidayUpdate = ...,
                   db: Session = Depends(get_db), _admin=Depends(get_current_admin)):
    try:
...
def delete_holiday(holidayId: int = Path(..., ge=1), db: Session = Depends(get_db), _admin=Depends(get_current_admin)):
    try:
...
def get_insurance_rates(db: Session = Depends(get_db), _admin=Depends(get_current_admin)):
    return services.get_insurance_rates(db)

@router.post("/insurance-rates", response_model=schemas.InsuranceRateOut, status_code=status.HTTP_201_CREATED)
def set_insurance_rate(payload: schemas.InsuranceRateSet, db: Session = Depends(get_db), _admin=Depends(get_current_admin)):
